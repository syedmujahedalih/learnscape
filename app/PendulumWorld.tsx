...
export default function PendulumWorld({ length, mass, angle, damping, setLength, setMass, setAngle, setDamping, locked, onExperiment }: Props) {
  ...
  return <div className="world-layout pendulum-world-layout"><section className="world-canvas pendulum-canvas">
    ...
    <label>Length <b>{length.toFixed(1)} m</b><input disabled={locked} aria-label="Pendulum length" type="range" min=".7" max="2.3" step=".1" value={length} onChange={event => setLength(Number(event.target.value))}/></label>
