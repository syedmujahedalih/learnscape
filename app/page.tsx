...
function Foundations({onPlay,onIncident}:{onPlay:()=>void;onIncident:()=>void}) {
  ...
  const chooseLesson = (index:number) => { setActiveIndex(index); setRan(false); };
  ...
  return <section className="learn-view">
    ...
    <div className="learn-shell">
      ...
      <article className="lesson-stage">
        ...
        {!ran ? <button className="run-concept" onClick={()=>setRan(true)}><span>RUN THE EXPERIMENT</span><b>{lesson.changeLabel}</b><i>→</i></button> : <div className="concept-result"><span>WHAT CHANGED</span><p>{lesson.takeaway}</p><div><button onClick={()=>setRan(false)}>REPLAY</button><button onClick={nextLesson}>{activeIndex === conceptLessons.length - 1 ? "OPEN FREE PLAYGROUND →" : "NEXT CONCEPT →"}</button></div></div>}
      </article>
    </div>
...
function Playground({onLearn,onIncident}:{onLearn:()=>void;onIncident:()=>void}) {
  ...
  const updatePlay = <K extends keyof InferenceConfig>(key:K,value:InferenceConfig[K]) => setPlayConfig(current=>({...current,[key]:value}));
  ...
  return <section className="play-view">
    ...
    <div className="play-shell">
      ...
      <aside className="play-guide"><div className="panel-index">03 / MAKE SENSE OF IT</div><h2>Try these experiments</h2><button onClick={()=>setPlayConfig({...initialConfig,precision:"FP16",cacheGb:14})}><span>MEMORY CLIFF</span><b>Can a larger KV cache cause an OOM?</b><i>LOAD →</i></button><button onClick={()=>setPlayConfig({...initialConfig,precision:"INT8",batchSize:1,concurrency:4})}><span>EMPTY GPU</span><b>What does under-batching do to the queue?</b><i>LOAD →</i></button><button onClick={()=>setPlayConfig({precision:"INT4",batchSize:8,cacheGb:10,concurrency:12,prefixCache:true,speculative:true})}><span>SLO HEADROOM</span><b>How do several optimizations compound?</b><i>LOAD →</i></button><div className="graduate-card"><span>READY FOR PRESSURE?</span><b>Take this mental model into a production incident.</b><button onClick={onIncident}>ENTER MISSION 01 →</button></div><button className="back-foundations" onClick={onLearn}>← REVIEW FOUNDATIONS</button></aside>
    </div>
  </section>;
}
