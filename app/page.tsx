...
const conceptLessons: ConceptLesson[] = [
  { id: "tail-latency", index: "01", title: "Tail latency", eyebrow: "START HERE · 3 MIN", summary: "Why averages hide the requests users remember.", definition: "P95 is the response time that 95% of requests finish within. Tail percentiles expose queueing and outliers that an average can hide.", question: "If a shared prompt is reused, which latency signal should move first?", choices: ["Time to first token", "Decode tokens per second", "Model answer quality"], answer: 0, change: "Enable prefix caching", observe: "TTFT and P95 latency", takeaway: "Prefix caching avoids repeated prefill work. Expect time to first token to improve before decode throughput changes." },
  { id: "batching", index: "02", title: "Continuous batching", eyebrow: "THROUGHPUT · 4 MIN", summary: "Keep the GPU fed without losing control of latency.", definition: "A scheduler groups work from active requests. More batching can raise throughput, but queueing and memory pressure can erase the gain.", question: "Will a larger batch always improve the run?", choices: ["Yes, in every workload", "Only until queueing or memory becomes the bottleneck", "No, batching never helps"], answer: 1, change: "Increase batch 1 → 8", observe: "Throughput, queue depth, and P95", takeaway: "Batching is a workload-dependent tradeoff. Measure throughput and tail latency together instead of treating either one as the answer." },
  { id: "kv-cache", index: "03", title: "KV cache", eyebrow: "MEMORY · 4 MIN", summary: "Trade accelerator memory for decoding capacity.", definition: "The KV cache stores attention state for tokens already processed. It competes with model weights and runtime allocations for device memory.", question: "What loses headroom when the cache budget grows?", choices: ["Network bandwidth", "GPU memory for weights and runtime work", "Prompt quality"], answer: 1, change: "Grow cache 6 GB → 14 GB", observe: "Peak VRAM and failed requests", takeaway: "Cache capacity is not free. Measure peak device memory under realistic sequence lengths and concurrency." },
  { id: "quantization", index: "04", title: "Quantization", eyebrow: "PRECISION · 4 MIN", summary: "Use fewer bits per weight to change memory and speed.", definition: "Quantization compresses model weights. It can reduce VRAM and improve throughput, while introducing a quality tradeoff that a performance trace alone cannot evaluate.", question: "What should you evaluate after switching to four-bit weights?", choices: ["Only response quality", "Nothing else", "Performance and quality as separate measurements"], answer: 2, change: "Switch FP16 → INT4", observe: "VRAM, throughput, latency, then quality separately", takeaway: "A faster trace does not prove acceptable model quality. Performance and evaluation belong in the same decision, but they are separate measurements." },
  { id: "concurrency", index: "05", title: "Concurrency", eyebrow: "SCHEDULING · 3 MIN", summary: "Control how many requests compete at once.", definition: "A concurrency cap limits active sequences. Too low can waste the GPU; too high can increase cache pressure and destabilize tail latency.", question: "How should you choose a concurrency cap?", choices: ["Use the highest value available", "Tune queueing, VRAM, and latency together", "Always use four"], answer: 1, change: "Raise concurrency 4 → 12", observe: "GPU utilization, queue depth, P95, and VRAM", takeaway: "Concurrency should be tuned with batching, sequence length, and cache capacity, not in isolation." },
  { id: "speculative", index: "06", title: "Speculative decoding", eyebrow: "DECODING · 5 MIN", summary: "Draft tokens cheaply and verify them with the target model.", definition: "Speculative decoding can reduce time per accepted token when the draft path is accurate enough to justify verification overhead.", question: "Will speculative decoding improve this exact deployment?", choices: ["Always", "Never", "Only a measured run can establish that"], answer: 2, change: "Enable speculative decoding", observe: "Accepted tokens, throughput, P95, and power", takeaway: "The gain depends on acceptance rate, implementation, model pair, and workload. This is exactly why the experiment needs a real runtime." },
];
...
function Foundations({ onPlay, onIncident }: { onPlay: () => void; onIncident: () => void }) {
  ...
  const chooseLesson = (index: number) => { setIntroComplete(true); setActiveIndex(index); setRevealed(false); setLessonAnswer(null); trackEvent("lesson_selected", { label: conceptLessons[index].id }); };
  ...
  return <section className="learn-view">
    <div className="learn-head"><div><span>GUIDED PATH · FOUNDATIONS</span><h1>Build the mental model.</h1><p>Follow one request first. Then learn how each serving decision changes it.</p></div><div className="course-progress"><span>{introComplete ? `${activeIndex + 1} / ${conceptLessons.length} LABS` : "60-SECOND PRIMER"}</span><i><em style={{ width: introComplete ? `${((activeIndex + 1) / conceptLessons.length) * 100}%` : "8%" }}/></i></div></div>
    {!introComplete ? <RequestPrimer onComplete={() => setIntroComplete(true)}/> : <>
    <div className="learn-shell">
      <aside className="lesson-list">{conceptLessons.map((item, index) => <button key={item.id} className={index === activeIndex ? "active" : ""} aria-current={index === activeIndex ? "step" : undefined} onClick={() => chooseLesson(index)}><span>{item.index}</span><div><b>{item.title}</b><small>{item.summary}</small></div><i>→</i></button>)}</aside>
      <article className="lesson-stage">
        <div className="lesson-copy"><span>{lesson.eyebrow}</span><h2>{lesson.title}</h2><p>{lesson.definition}</p><div className="lesson-question"><small>COMMIT TO A HYPOTHESIS</small><b>{lesson.question}</b><div className="lesson-choices" role="radiogroup" aria-label={lesson.question}>{lesson.choices.map((choice, index) => <button type="button" role="radio" aria-checked={lessonAnswer === index} key={choice} className={lessonAnswer === index ? "selected" : ""} onClick={() => { setLessonAnswer(index); setRevealed(false); }}>{choice}</button>)}</div></div></div>
        ...
        {!revealed ? <button className="run-concept" disabled={lessonAnswer === null} onClick={() => { setRevealed(true); trackEvent("experiment_planned", { label: lesson.id, value: String(lessonAnswer) }); }}><span>{lessonAnswer === null ? "CHOOSE AN ANSWER FIRST" : "CHECK MY REASONING"}</span><b>{lesson.change}</b><i>→</i></button> : <div className="concept-result" aria-live="polite"><span>{lessonAnswer === lesson.answer ? "YOUR HYPOTHESIS IS ON TRACK" : "YOUR HYPOTHESIS WAS CHALLENGED"}</span><p>{lesson.takeaway}</p><div><button onClick={() => { setRevealed(false); setLessonAnswer(null); }}>TRY AGAIN</button><button onClick={nextLesson}>{activeIndex === conceptLessons.length - 1 ? "BUILD YOUR OWN EXPERIMENT →" : "NEXT CONCEPT →"}</button></div></div>}
      </article>
    </div>
...
function RequestPrimer({ onComplete }: { onComplete: () => void }) {
  const stages = [
    ["01", "Request", "A prompt arrives at the serving endpoint."],
    ["02", "Queue", "The scheduler waits, groups, and admits work."],
    ["03", "Prefill", "The GPU reads the prompt. TTFT starts here."],
    ["04", "Decode", "The model generates tokens one step at a time."],
    ["05", "Stream", "The user receives the answer. P95 sees the whole trip."],
  ];
  return <section className="primer" aria-labelledby="primer-title"><div className="primer-copy"><span>60-SECOND PRIMER</span><h2 id="primer-title">Follow one request.</h2><p>Inference engineering is the work of moving a request through a serving system quickly, reliably, and within memory. Every lab changes one part of this path.</p><div className="primer-glossary"><div><b>TTFT</b><span>Wait until the first token</span></div><div><b>THROUGHPUT</b><span>Tokens produced each second</span></div><div><b>P95</b><span>The slower edge of user experience</span></div><div><b>KV CACHE</b><span>Memory that keeps prior token state</span></div></div><button onClick={onComplete}>I’M READY FOR LAB 01 <span>→</span></button></div><div className="request-path" aria-label="Request lifecycle">{stages.map(([number, title, detail], index) => <article key={title}><span>{number}</span><div><b>{title}</b><small>{detail}</small></div>{index < stages.length - 1 && <i>↓</i>}</article>)}</div></section>;
}
...
function Playground({ cloudConfigured, onLearn, onIncident }: { cloudConfigured: boolean; onLearn: () => void; onIncident: () => void }) {
  ...
  return <section className="play-view">
    ...
    <div className="play-shell">
      <aside className="play-controls"><div className="panel-index">01 / CONFIGURE</div><h2>Your serving experiment</h2><Control label="WEIGHT PRECISION" hint="memory ↔ quality"><Segment label="Weight precision" values={["FP16", "INT8", "INT4"]} selected={config.precision} onSelect={value => update("precision", value as Precision)}/></Control><Control label="CONTINUOUS BATCH" hint="latency ↔ throughput"><Segment label="Continuous batch size" values={[1, 8, 16]} selected={config.batchSize} onSelect={value => update("batchSize", value as BatchSize)} prefix="B"/></Control><Control label="KV CACHE BUDGET" hint="capacity"><Segment label="KV cache budget" values={[6, 10, 14]} selected={config.cacheGb} onSelect={value => update("cacheGb", value as CacheSize)} suffix="GB"/></Control><Control label="CONCURRENCY CAP" hint="queue pressure"><Segment label="Concurrency cap" values={[4, 12, 24]} selected={config.concurrency} onSelect={value => update("concurrency", value as Concurrency)} prefix="C"/></Control><Toggle label="PREFIX CACHING" detail="Reuse the shared prompt" checked={config.prefixCache} onChange={value => update("prefixCache", value)}/><Toggle label="SPECULATIVE DECODING" detail="Draft, then verify tokens" checked={config.speculative} onChange={value => update("speculative", value)}/><button className="play-reset" onClick={() => setConfig({ ...defaultExperimentConfig, precision: "INT8", batchSize: 8 })}>RESET CONFIGURATION</button><p className="sr-only" aria-live="polite">Current configuration: {config.precision}, batch {config.batchSize}, {config.cacheGb} gigabyte cache, concurrency {config.concurrency}, prefix caching {config.prefixCache ? "on" : "off"}, speculative decoding {config.speculative ? "on" : "off"}.</p></aside>
      <section className="play-output"><div className="play-output-head"><div><span>02 / EXPERIMENT SPEC</span><h2>Ready to reproduce</h2></div><div className="play-status"><i/> AWAITING RUNTIME</div></div><div className="play-trajectory"><div className="trajectory-head"><span>MEASUREMENT PIPELINE</span><b>No estimated values</b></div><div className="concept-pipeline"><span>CONFIG</span><i>→</i><b>RUNTIME</b><i>→</i><b>TRACE</b><i>→</i><span>COMPARE</span></div></div><div className="play-metrics"><Metric label="MODEL" value="7B" delta={benchmarkWorkload.model}/><Metric label="PROMPT" value={`${benchmarkWorkload.promptTokens}`} suffix="tok" delta={`${benchmarkWorkload.sharedPrefix}% shared prefix`}/><Metric label="OUTPUT" value={`${benchmarkWorkload.outputTokens}`} suffix="tok" delta="per request"/><Metric label="RUN" value="24" suffix="sec" delta="controlled workload"/></div><div className="causal-readout"><span>CONFIGURATION TO TEST</span><h3>{config.precision} · batch {config.batchSize} · {config.cacheGb} GB cache · concurrency {config.concurrency}</h3><p>Prefix caching {config.prefixCache ? "enabled" : "disabled"}. Speculative decoding {config.speculative ? "enabled" : "disabled"}. Keep the workload, model, and hardware fixed when comparing this configuration.</p></div><button className="run-concept" onClick={() => void copySpec()} aria-describedby="copy-status"><span>{copied ? "COPIED" : "COPY EXPERIMENT SPEC"}</span><b>JSON</b><i>→</i></button><span id="copy-status" className="sr-only" aria-live="polite">{copied ? "Experiment specification copied to the clipboard." : ""}</span></section>
...
function Incident({ cloudConfigured, onExit }: { cloudConfigured: boolean; onExit: () => void }) {
  ...
  return <section className="incident-view">
    ...
    {!cloudConfigured && <section className="runner-gate" role="status"><div><span>RUNTIME REQUIRED</span><h2>This lab needs connected compute.</h2><p>You can inspect the incident and preview a configuration. A measured run stays locked until an operator attaches a GPU runner.</p></div><a href="https://github.com/syedmujahedalih/learnscape/blob/main/docs/cloud-benchmarks.md" target="_blank" rel="noreferrer">CONNECT A RUNNER →</a></section>}
    <div className="incident-grid">
      ...
      <aside className={`control-panel ${!cloudConfigured ? "disconnected" : ""}`}><div className="panel-index">02 / {cloudConfigured ? "CONFIGURE" : "PREVIEW"}</div><h2>{cloudConfigured ? "Choose, then measure." : "Preview the serving setup."}</h2>{cloudConfigured && <Control label="CONNECTED GPU" hint="real environment"><Segment label="Connected GPU" values={["T4", "L4", "A10"]} selected={gpu} onSelect={value => { setGpu(value as CloudGpu); setResult(null); }}/></Control>}<Control label="WEIGHT PRECISION" hint="runtime setting"><Segment label="Weight precision" values={["FP16", "INT8", "INT4"]} selected={config.precision} onSelect={value => update("precision", value as Precision)}/></Control><Control label="CONTINUOUS BATCH" hint="runtime setting"><Segment label="Continuous batch size" values={[1, 8, 16]} selected={config.batchSize} onSelect={value => update("batchSize", value as BatchSize)} prefix="B"/></Control><Control label="KV CACHE" hint="budget"><Segment label="KV cache budget" values={[6, 10, 14]} selected={config.cacheGb} onSelect={value => update("cacheGb", value as CacheSize)} suffix="GB"/></Control><Toggle label="PREFIX CACHING" detail="Reuse shared prefixes" checked={config.prefixCache} onChange={value => update("prefixCache", value)}/><Toggle label="SPECULATIVE DECODING" detail="Draft, then verify" checked={config.speculative} onChange={value => update("speculative", value)}/><div className="prediction-block"><span>COMMIT TO AN OUTCOME</span><div role="radiogroup" aria-label="Predicted measured outcome"><button type="button" role="radio" aria-checked={prediction === "clear"} className={prediction === "clear" ? "selected" : ""} onClick={() => setPrediction("clear")}>SLOS CLEAR</button><button type="button" role="radio" aria-checked={prediction === "latency"} className={prediction === "latency" ? "selected" : ""} onClick={() => setPrediction("latency")}>LATENCY MISSES</button><button type="button" role="radio" aria-checked={prediction === "oom"} className={prediction === "oom" ? "selected" : ""} onClick={() => setPrediction("oom")}>GPU OOMS</button></div></div>{error && <p className="cloud-error">{error}</p>}<div className="run-actions"><button className="deploy-button" disabled={!prediction || !cloudConfigured || running} onClick={() => void run()}><span>{running ? "RUNNING ON CONNECTED GPU" : cloudConfigured ? "RUN MEASURED TRACE" : "CONNECT RUNNER TO MEASURE"}</span><i>→</i></button></div><small className="deploy-note">{cloudConfigured ? "The lab records latency, throughput, queue, utilization, power, and memory when the runner provides them." : "No synthetic values are substituted while the runner is disconnected."}</small></aside>
    </div>
  </section>;
}
...
function Control({ label, hint, children }: { label: string; hint: string; children: ReactNode }) { return <fieldset className="control-group"><legend><span>{label}</span><i>{hint}</i></legend>{children}</fieldset>; }
function Segment({ label, values, selected, onSelect, prefix = "", suffix = "" }: { label: string; values: (string | number)[]; selected: string | number; onSelect: (value: string | number) => void; prefix?: string; suffix?: string }) { return <div className="segment" role="radiogroup" aria-label={label}>{values.map(value => <button type="button" role="radio" aria-checked={selected === value} key={value} className={selected === value ? "selected" : ""} onClick={() => onSelect(value)}>{prefix}{value}{suffix}</button>)}</div>; }
function Toggle({ label, detail, checked, onChange }: { label: string; detail: string; checked: boolean; onChange: (value: boolean) => void }) { return <button type="button" role="switch" aria-checked={checked} aria-label={`${label}. ${detail}`} className={`infra-toggle ${checked ? "on" : ""}`} onClick={() => onChange(!checked)}><span><b>{label}</b><small>{detail}</small></span><i aria-hidden="true"><em/></i></button>; }
