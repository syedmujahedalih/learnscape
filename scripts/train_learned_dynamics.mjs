...
const outputFile = path.join(root, "lib", "inference", "learned-dynamics-weights.ts");
const inputSize = 19;
const hiddenSize = 24;
const outputSize = 6;
...
function syntheticSimulatorSamples() {
  const samples = [];
...
const samples = real.length >= 200 ? real : syntheticSimulatorSamples();
const source = real.length >= 200 ? "cloud_and_local_traces" : "simulator_synthetic";
...
fs.writeFileSync(outputFile, `// Generated by scripts/train_learned_dynamics.mjs\nexport const learnedDynamicsWeights = ${JSON.stringify(artifact)} as const;\n`);
